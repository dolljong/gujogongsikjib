...
- [x] **I(단면2차모멘트) 입력에 단면 선택 팝업** — `SectionPicker.tsx`, `sections.ts`
      I칸 모드 `[직접입력/단면 선택]`. 팝업에서 단면성능 53종 중 선택→제원 입력→
      A·e·I·Z·i 즉시 계산(기존 엔진 재사용)→`[I에 적용]`으로 값·단위 자동 채움.
      복붙 없이 단위 자동 일치. 브라우저 검증: 직사각형 b300·h500→I=3.125e9 mm⁴,
      적용 후 δ=0.144 mm. 45/53 정상, 8개는 원본 데이터 이슈(적용버튼 미표시로 degrade).
- [~] 단면공식 원본 이슈 8건 → 2건 처리, 7건 잔여(오타 아님):
      · [x] 1.6.11 중공사각형 I식 `h_i`→`h_1` 오타 — `import_excel.py` LATEX_FIXES 보정+재생성.
        검증: b300·h500·b1 260·h1 460 → I=1.0161e9 mm⁴.
      · [x] `\leftrightharpoons`(≒ 근사기호, 1.6.18/19) — `latex.js`에서 근사식 뒷부분 제거.
      · [x] 1.6.18/19 다각형·1.6.42/43 부채꼴 — 원본 스캔 확인 후 처리 완료.
        `sections.ts` SECTION_DIM_OVERRIDE(그림변수 차원 명시: 길이/각도/개수),
        SectionPicker 각도=도(°)입력→라디안 환산. `latex.js` `\sin 2\phi`→sin(2φ)
        인수보정(계수+기호). 검증: 육각형 I=5.4127e7(이론값), 부채꼴 I=7.135e6/3.5675e6.
      · [ ] 1.6.44/45 활꼴 — I식 sin³α·복잡 중괄호 구조로 미계산(트랜스파일 한계).
      · [ ] 1.6.51/52/53 형강 — 데이터 공백(51: e₂=H−e₁ 정의 누락, 52: b_{1}h_{1} 변수병합,
        53: 삼각식). 단위는 길이로 처리되나 I 미계산 → 형상 유도식/전용처리 필요.
